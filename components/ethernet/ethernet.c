...
static void apply_static_ip(esp_netif_t *netif)
{
    ...
    if (NET.use_dhcp) {
        esp_err_t er = esp_netif_dhcpc_start(netif);
        if (er != ESP_OK && er != ESP_ERR_ESP_NETIF_DHCP_ALREADY_STARTED) {
            ESP_LOGW(TAG, "esp_netif_dhcpc_start() failed: %s", esp_err_to_name(er));
        }
        return;
    }
